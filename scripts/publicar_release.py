"""Publica una release de DracPaste y actualiza el manifiesto de actualizaciones.
...
# DracPaste es un monorepo con las dos mitades: el proyecto Android cuelga de android/,
# no de la raíz como en las demás apps. Todo lo demás (docs/, scripts/) sí vive arriba.
...
# El instalador de Windows, si se ha generado con scripts/publicar-windows.ps1. Viaja en
# la misma Release que el APK: las dos mitades de DracPaste son la misma versión y
# separarlas invitaría a instalar una v1.2 en el PC contra una v1.1 en el móvil.
INSTALADOR_WINDOWS = RAIZ / "dist"

_REPO = "marcmayol/dracpaste"
_PAGES_URL = "https://marcmayol.com/dracpaste/updates.json"
_CHECK_HORAS = 24
_ENV_FIRMA = (
    "DRACPASTE_STORE_FILE",
    "DRACPASTE_STORE_PASSWORD",
    "DRACPASTE_KEY_ALIAS",
    "DRACPASTE_KEY_PASSWORD",
)
...
def url_release(version_name: str) -> str:
    return (
        ...
        f"v{version_name}/dracpaste-v{version_name}.apk"
    )
...
def construir() -> Path:
    ...
    # Antes de empaquetar nada, que el protocolo siga cuadrando. Es barato y evita
    # publicar una versión que no se entiende con el lado de Windows.
    _ejecutar([_gradlew(), ":protocolo:test", ":app:testDebugUnitTest"], cwd=str(ANDROID))

    # cwd=ANDROID: el gradlew de este monorepo no está en la raíz del repositorio.
    _ejecutar([_gradlew(), ":app:assembleRelease"], cwd=str(ANDROID))
...
def _asset_con_nombre(apk: Path, vn: str) -> Path:
    destino = apk.with_name(f"dracpaste-v{vn}.apk")
    if destino != apk:
...
def _instalador_de_windows(vn: str) -> Path | None:
    """
    El .exe que genera scripts/publicar-windows.ps1, si está y es de esta versión.

    No se compila aquí: publicar el APK no debería depender de tener el SDK de .NET a
    mano. Si falta, se avisa y la Release sale solo con el APK; el instalador se puede
    subir después con `gh release upload`.
    """
    if not INSTALADOR_WINDOWS.is_dir():
        return None

    candidato = INSTALADOR_WINDOWS / f"DracPaste-{vn}-instalador.exe"
    if candidato.is_file():
        return candidato

    sueltos = sorted(INSTALADOR_WINDOWS.glob("DracPaste-*-instalador.exe"))
    if sueltos:
        print(
            f"  Aviso: hay instaladores en dist/ pero ninguno de la v{vn} "
            f"({', '.join(p.name for p in sueltos)}). La Release irá solo con el APK."
        )
    return None
...
def publicar(apk: Path, manifiesto: dict, notas: str) -> None:
    ...
    # Las dos mitades de DracPaste van en la misma Release: son la misma versión, y
    # separarlas invitaría a acabar con una v1.2 en el PC contra una v1.1 en el móvil.
    assets = [str(asset)]
    instalador = _instalador_de_windows(vn)
    if instalador is not None:
        assets.append(str(instalador))
    else:
        print("  Aviso: no se publica instalador de Windows en esta Release.")

    _ejecutar([
        "gh", "release", "create", f"v{vn}", *assets,
        "--repo", _REPO,
        "--title", f"DracPaste {vn}",
        "--notes", notas or f"DracPaste {vn}.",
    ])
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Publica una release de DracPaste.")
